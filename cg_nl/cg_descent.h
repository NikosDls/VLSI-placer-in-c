...
/*
  ...
  http://users.clas.ufl.edu/hager/papers/Software/
	...
	of using floats and integers (double and longs by default).
...
/* If the CG_BLAS are not installed, then the following definitions
	 can be ignored. If the CG_BLAS are available, then to use them,
	 comment out the the next statement (#define CG_NO_BLAS) and make
	 ...
	 benefit from using unrolled CG_BLAS. There could be a benefit from
	 using threaded CG_BLAS if the problems is really big. However,
	 performing low dimensional operations with threaded CG_BLAS can be
...
/* if CG_BLAS are used, specify the integer precision */
#define CG_BLAS_INT long int

/* if CG_BLAS are used, comment out the next statement if no
...
/* only use idamax when the vector size >= CG_IDAMAX_START */
#define CG_IDAMAX_START 25

/* only use matrix CG_BLAS for transpose multiplication when number of
...
CG_BLAS_INT CG_IDAMAX (CG_BLAS_INT *n, CG_FLOAT *DX, CG_BLAS_INT *incx) ;
...
typedef struct cg_parameter_struct /* user controlled parameters */
{
	...
	/* |f| < SmallCost*starting cost => skip QuadStep and set PertRule = CG_FALSE*/
	CG_FLOAT SmallCost ;
...
)
{
	...
	int     l1, l2, j, k, mem, memsq = 0, memk, memk_begin = 0, mlast, mlast_sub,
					mp, mp_begin = 0, mpp, nsub, spp, spp1, SkFstart = 0, SkFlast = 0,
					Subspace, UseMemory, Restart, LBFGS, InvariantSpace,
					IterSub = 0, NumSub = 0,
	...
	CG_FLOAT  gHg, scale, gsubnorm2 = 0,  ratio, stgkeep = 0,
					...
					*Yk = NULL, *SkYk = NULL, *dsub = NULL, *gsub = NULL,
					*gsubtemp = NULL, *gkeep = NULL, *tau = NULL, *vsub = NULL,
					*wsub = NULL ;
	...
	Com.Wolfe = CG_FALSE ; /* initially Wolfe line search not performed */
	Com.nf = 0 ;  /* number of function evaluations */
	Com.ng = 0 ;  /* number of gradient evaluations */
  CG_CUSTOM_INIT(Com)
	iter = 0 ;    /* total number of iterations */
	QuadF = CG_FALSE ;     /* initially function assumed to be nonquadratic */
	NegDiag = CG_FALSE ;   /* no negative diagonal elements in QR factorization */
	...
	LBFGS = CG_FALSE ;
	UseMemory = CG_FALSE ;/* do not use memory */
	Subspace = CG_FALSE ; /* full space, check subspace condition if UseMemory */
	FirstFull = CG_FALSE ;/* not first full iteration after leaving subspace */
	...
	if ( mem > 0 )
	{
		...
		{
			UseMemory = CG_TRUE ; /* previous search direction will be saved */
			...
			InvariantSpace = CG_FALSE ; /* iterations not in invariant space */
	...
	Restart = CG_FALSE ;    /* do not restart the algorithm */
	...
	for (iter = 1; iter <= maxit; iter++)
	{
		...
		Com.UseCubic = CG_TRUE ;
		if ( (t < Parm->CubicCutOff) || !Parm->UseCubic ) Com.UseCubic = CG_FALSE ;
		if ( Parm->QuadStep )
		{
		...
		if ( IterQuad == qrestart ) QuadF = CG_TRUE ;
		IterRestart++ ;
		if ( !Com.AWolfe )
		{
		...
		if ( !UseMemory )
		{
			if ( !LBFGS )
			{
				if ( (IterRestart >= nrestart) || ((IterQuad == qrestart)
							&& (IterQuad != IterRestart)) ) Restart = CG_TRUE ;
			}
		}
		else
		{
			if ( Subspace ) /* the iteration is in the subspace */
			{
				...
				else
				{
					...
					if ( IterSubRestart == nrestartsub ) Restart = CG_TRUE ;
				}
			}
			else  /* in full space */
			{
				...
				gnorm2 = cg_dot (gtemp, gtemp, n) ;
				ratio = sqrt (gsubnorm2/gnorm2) ;
				if ( ratio > 1-Parm->eta2) InvariantSpace = CG_TRUE ;
						...
						((memk == mem) && (ratio > 1-Parm->eta0)) )
				{
					...
					if ( d0isg && !memk_is_mem ) DenseCol1 = CG_FALSE ;
					else                         DenseCol1 = CG_TRUE ;
		...
		else  /* compute the search direction in the full space */
		{
			...
			else /* FirstFull = CG_TRUE, precondition after leaving subspace */
			{
				/* set x = xtemp */
				cg_copy (x, xtemp, n) ;
...
)
{
	...
	else if ( gnorm <= Com->tol*(1 + fabs (Com->f)) ) return (1) ;
	return (0) ;
}
...
Line:
	...
	for (iter = 0; iter < Parm->nline; iter++)
	{
		...
		if ( (alpha <= a) || (alpha >= b) )
		{
			...
			Com->QuadOK = CG_FALSE ; /* bisection was used */
		}
...
)
{
	...
	for (iter = 0; iter < Parm->nshrink; iter++)
	{
		if ( (toggle == 0) || ((toggle == 2) && ((b-a) <= width)) )
		{
			...
			toggle = 0 ;
			width = Parm->gamma*(b-a) ;
			if ( iter ) Com->QuadOK = CG_TRUE ; /* at least 2 cubic iterations */
		}
		else if ( toggle == 1 )
		{
			Com->QuadOK = CG_TRUE ;
	...
	if ( fabs (fb) <= Com->SmallCost ) Com->PertRule = CG_FALSE ;
...
/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	 Start of routines that could use the CG_BLAS
	 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
...
/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	 End of routines that could use the CG_BLAS
	 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
...
Version 6.0 Changes:
  ...
  It is now possible to utilize the CG_BLAS, if they are available, by
